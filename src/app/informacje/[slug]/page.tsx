  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      */}
      ...
      <section className="relative w-full h-[60vh] min-h-[400px] flex items-end justify-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[900px] mx-auto px-6 pb-16 animate-[fadeInUp_0.8s_ease-out]">
          <Link href="/informacje" className="inline-flex items-center text-white/70 hover:text-white transition-colors mb-6 text-sm font-medium">
            <ArrowLeft className="w-4 h-4 mr-2" /> Wróć do huba informacji
          </Link>
      ...
      <section className="relative w-full max-w-[900px] mx-auto px-6 py-16 -mt-8 bg-white md:rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-20">
        
        {/* Intro / Excerpt */}
        <p className="text-xl md:text-2xl text-slate-700 font-medium leading-relaxed mb-12 border-l-4 border-brand-green pl-6 italic">
          {article.excerpt}
        </p>
        ...
        <article 
          ...
          [&>h2]:text-3xl [&>h2]:font-extrabold [&>h2]:text-slate-900 [&>h2]:mt-12 [&>h2]:mb-6 
          ...
          [&>a]:text-brand-green [&>a]:font-medium hover:[&>a]:text-brand-dark transition-colors"
        ...
        <div className="mt-16 pt-8 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            <button className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-brand-green hover:text-white transition-colors">
      ...
      <section className="w-full bg-slate-100 py-20 px-6">
        <div className="max-w-[1200px] mx-auto">
          <h3 className="text-2xl font-extrabold text-slate-900 mb-10 text-center">
            Poczytaj również w naszej strefie
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {relatedArticles.map((rel) => (
              <Link key={rel.id} href={`/informacje/${rel.slug}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col">
                <div className="relative h-48 w-full overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-brand-green/90 text-white text-[10px] uppercase font-bold tracking-widest rounded-full backdrop-blur-md">
                      {rel.category}
                    </span>
                  </div>
                </div>
                <div className="p-6 flex-1 flex flex-col">
                  <h4 className="font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-brand-green transition-colors">
                    {rel.title}
                  </h4>
                  <div className="mt-auto flex items-center justify-between text-sm font-medium">
                    <span className="text-slate-400">{rel.date}</span>
                    <span className="text-brand-green flex items-center">Czytaj <ChevronRight className="w-4 h-4 ml-1 transition-transform group-hover:translate-x-1" /></span>
                  </div>
                </div>
              </Link>
            ))}
